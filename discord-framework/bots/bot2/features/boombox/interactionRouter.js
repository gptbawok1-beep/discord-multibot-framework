...
import { buildSetupBoomBoxPanel, buildDeleteConfirmPanel, buildMonitorEmbed } from "./setup/panel.js";
...
export async function handleBoomBoxInteractionRouter(interaction) {
  ...
  // ── 4. bbsetup: interactions (setup subpanels) ──────────────────────────────
  if (id.startsWith("bbsetup:")) {
    try {
      // Setup Main back-home & close
      if (id === "bbsetup:back") {
      ...
      if (id === "bbsetup:delete:confirm") {
        ...
        await interaction.update({ embeds: [
          ...
          new ActionRowBuilder().addComponents(
            new ButtonBuilder().setCustomId("bbsetup:back").setLabel("Menu Utama").setStyle(ButtonStyle.Success)
          )
        ] });
        return true;
      }
      ...
      if (durModalSubmitMatch && interaction.isModalSubmit()) {
        const roleId = durCustomMatch ? durCustomMatch[1] : id.split(":").pop();
